...
# CONFIGURATION
# =============================================================================
IMAGE_NAME="taaos-builder:wsl"
CONTAINER_NAME="taaos_wsl_factory_$(date +%s)"
TAR_NAME="TaaOS-WSL"
...
# =============================================================================
echo "╔═══════════════════════════════════════════════════════════════════════╗"
echo "║                   TaaOS WSL Build - TERMINAL EDITION                  ║"
echo "╚═══════════════════════════════════════════════════════════════════════╝"
...
docker rm -f $(docker ps -a -q -f name=taaos_wsl_factory) 2>/dev/null || true
...
docker exec -w /build "$CONTAINER_NAME" bash -c '
    ...
    # Build Chroot and Package Tarball
    echo "=== PHASE C: Building Chroot for WSL ==="
    lb bootstrap
    lb chroot
    
    echo "=== PHASE D: Injecting WSL Config & First-Boot Setup ==="
    if [ -d chroot ]; then
        # 1. Base wsl.conf (Systemd enabled, default is root for the first boot)
        mkdir -p chroot/etc
        cat > chroot/etc/wsl.conf << 'WSLCONF'
[boot]
systemd=true
WSLCONF

        # 2. First-Boot OOBE Script
        cat > chroot/usr/local/bin/taaos-wsl-setup << 'OOBESCRIPT'
#!/bin/bash
if [ -f /etc/taaos-wsl-configured ]; then
    exit 0
fi

echo -e "\033[0;32m"
echo "╔═══════════════════════════════════════════════════════════════════════╗"
echo "║                 Welcome to TaaOS - Terminal Edition                   ║"
echo "╚═══════════════════════════════════════════════════════════════════════╝"
echo -e "\033[0m"
echo "Let's set up your personal account. You will have sudo (Admin) privileges."
echo ""

while true; do
    read -p "Enter your new username (lowercase only): " NEW_USER
    if [[ "$NEW_USER" =~ ^[a-z_][a-z0-9_-]*$ ]]; then
        break
    else
        echo -e "\033[0;31mInvalid username. Please use only lowercase letters and numbers.\033[0m"
    fi
done
...
# Grant sudo and other group privileges
usermod -aG sudo,docker,video,audio,plugdev "$NEW_USER" 2>/dev/null

# Set user as default for future logins
...
touch /etc/taaos-wsl-configured
echo ""
echo -e "\033[0;32mConfiguration Complete!\033[0m"
echo "Logging you in as $NEW_USER..."
exec su - "$NEW_USER"
OOBESCRIPT
        chmod +x chroot/usr/local/bin/taaos-wsl-setup

        # 3. Trigger OOBE on root login
        echo "/usr/local/bin/taaos-wsl-setup" >> chroot/root/.profile
        
        echo "WSL first-boot setup script injected successfully."
    fi
    
    echo "=== PHASE E: Packaging WSL Tarball ==="
    if [ -d chroot ]; then
        tar -czpf /build/TaaOS-WSL.tar.gz -C chroot .
    else
...
# Retrieve Tarball
echo "[6/6] Retrieving WSL Tarball..."
if [ $BUILD_RESULT -eq 0 ]; then
    docker cp "$CONTAINER_NAME":/build/TaaOS-WSL.tar.gz ./TaaOS-WSL.tar.gz 2>/dev/null || true
    
    docker rm -f "$CONTAINER_NAME"
    echo ""
    echo "╔═══════════════════════════════════════════════════════════════════════╗"
    echo "║                  SUCCESS! WSL Tarball is ready.                       ║"
    echo "╚═══════════════════════════════════════════════════════════════════════╝"
    ls -lh ./TaaOS-WSL.tar.gz 2>/dev/null || echo "Check current directory for TaaOS-WSL.tar.gz"
else
    echo ""
